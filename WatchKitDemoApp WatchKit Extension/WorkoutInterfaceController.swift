...
class WorkoutInterfaceController: WKInterfaceController, HKWorkoutSessionDelegate {
    ...
    // MARK: IB Actions
    @IBAction func didTappedPauseButton() {
...
//        }
        ...
        healthStore.save(totalDistanceSample, withCompletion: { (success, error) -> Void in
            if( error != nil ) {
                print(error ?? "fail")
            } else {
        ...
        healthStore.save(totalEnergyBurnedSample, withCompletion: { (success, error) -> Void in
            if( error != nil ) {
                print(error ?? "fail")
            } else {
                print("The energy has been recorded! Better go check!")
            }
        })
